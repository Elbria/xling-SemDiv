...
# Adapted from Hugging Face library to fine-tune on synthetic divergences using CE loss
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0 and not args.tpu:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if args.local_rank == -1 and args.evaluate_during_training:  # Only evaluate when single GPU otherwise metrics may not average well
                        ...
                        tb_writer.add_scalars('eval_loss_per_class',
                                              {'pos': results_eval['pos_loss'], 'neg': results_eval['neg_loss']},
                                              global_step)

                    tb_writer.add_histogram('eval_neg_loss_hist_' + str(global_step) , np.array(results_eval['neg_hist']), bins=100)
                    tb_writer.add_histogram('eval_pos_loss_hist_' + str(global_step) , np.array(results_eval['pos_hist']), bins=100)
...
def evaluate(args, model, tokenizer, checkpoint, evaluate_on_training=False, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        with open(output_pred_file, "w") as writer:
            for (sample_loss,(pr,gt)) in zip(eval_loss_total,zip(preds, out_label_ids)):
                if gt == 0:
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        elif args.evaluation_set == 'unlabeled_rn':
            examples = processor.get_unlabeled_rn_examples(args.data_dir) if evaluate else processor.get_train_examples(args.data_dir)
        elif args.evaluation_set == 'test_synth':
            examples = processor.get_test_synth_examples(args.synth_data_dir) if evaluate else processor.get_train_examples(args.data_dir)
        elif args.evaluation_set == 'unrelated':
            examples = processor.get_unrelated_examples(args.synth_data_dir) if evaluate else processor.get_train_examples(args.data_dir)
        elif args.evaluation_set == 'some_meaning_difference':
            examples = processor.get_some_meaning_difference_examples(args.synth_data_dir) if evaluate else processor.get_train_examples(args.data_dir)
        elif args.evaluation_set == 'no_meaning_difference':
            examples = processor.get_no_meaning_difference_examples(args.synth_data_dir) if evaluate else processor.get_train_examples(args.data_dir)
        if args.evaluation_set == 'test':
            examples = processor.get_test_examples(args.synth_data_dir) if evaluate else processor.get_train_examples(args.data_dir)

...
# Adapted from Hugging Face library to fine-tune on synthetic divergences using CE token level loss
...
class InputExample(object):
    ...
    def __init__(self, guid, src_words, tgt_words, src_labels, tgt_labels):
...
def read_examples_from_file(data_dir, mode):
    ...
    with open(file_path, encoding="utf-8") as f:
        for id_,line in enumerate(f):
            ...
            span_label_maps = {'CHA':'D', 'ADD':'D', 'O':'O','D':'D'}
            ...
            examples.append(InputExample(guid="%s-%d".format(mode, guid_index),
                                         src_words=div_src,
                                             tgt_words=div_tgt,
                                         src_labels=div_src_token,
                                         tgt_labels=div_tgt_token))
...
def convert_examples_to_features(examples,
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        tokens, label_ids, segment_ids = [], [], []

        # Token-level annotation for source sentence
        for word, label in zip(example.src_words, example.src_labels):
        ...
        segment_ids.extend([sequence_a_segment_id] * source_length)

        # Token-level annotation for source sentence
        for word, label in zip(example.tgt_words, example.tgt_labels):
            word_tokens = tokenizer.tokenize(word)
            tokens.extend(word_tokens)
            # Use the same id for all tokens of the word
            label_ids.extend([label_map[label]] * (len(word_tokens)))
        ...
        if sep_token_extra:
            ...
            tokens += [sep_token]
            label_ids += [pad_token_label_id]
        segment_ids.extend([sequence_b_segment_id] * (len(tokens)-source_length))

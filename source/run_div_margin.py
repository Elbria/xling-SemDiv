...
# Adapted from Hugging Face library to fine-tune on synthetic divergences using margin loss
...
def train(args, train_dataset, model, tokenizer):
    ...
    if args.local_rank in [-1, 0]:
        if args.experiment_identifier != '':
            tb_writer = SummaryWriter('runs/'+args.experiment_identifier)
        else:
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if args.model_type != 'distilbert':
                inputs['token_type_ids_a'] = batch[2] if args.model_type in ['bert', 'xlnet'] else None  # XLM, DistilBERT and RoBERTa don't use segment_ids
                inputs['token_type_ids_b'] = batch[6] if args.model_type in ['bert', 'xlnet'] else None  # XLM, DistilBERT and RoBERTa don't use segment_ids
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0 and not args.tpu:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if args.local_rank == -1 and args.evaluate_during_training:  # Only evaluate when single GPU otherwise metrics may not average well
                        results_eval = evaluate(args, model, tokenizer, global_step)

                    if args.local_rank == -1 and args.evaluate_on_training:  # Only evaluate when single GPU otherwise metrics may not average well
                        results_training = evaluate(args, model, tokenizer, global_step, evaluate_on_training=True)

                    tb_writer.add_scalar('lr', scheduler.get_last_lr()[0], global_step)
                    tb_writer.add_scalars('f1', {'training_f1':results_training['f1'], 'eval_f1':results_eval['f1']}, global_step)
                    tb_writer.add_scalars('loss', {'training_loss':results_training['total_loss'], 'eval_loss':results_eval['total_loss']}, global_step)
...
def evaluate(args, model, tokenizer, checkpoint, evaluate_on_training=False, evaluate_on_test=False, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        elif not evaluate_on_test:
            eval_dataset = load_and_cache_examples(args, eval_task, tokenizer, evaluate=True)
        else:
            eval_dataset = load_and_cache_examples(args, eval_task, tokenizer, evaluate=True, evaluate_on_test=True)
        ...
        for batch in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            if preds is None:
                ...
                if not evaluate_on_test:
                    preds = np.append(preds, logits_b.detach().cpu().numpy(), axis=0)
                    out_label_ids = np.append(out_label_ids, inputs['labels_b'].detach().cpu().numpy(), axis=0)
            else:
        ...
        #sigm=[float(x[0]) for x in preds]
        preds = [1 for x in range(len(sigm))]
        #sigm=[float(x[0]) for x in preds]
        ...
        output_pred_file = os.path.join(eval_output_dir,
                                        '{0}_{1}_preds_gt.txt'.format(str(checkpoint), str(args.evaluation_set)))
        
        with open(output_pred_file, "w") as writer:
            for (prob,(pr,gt)) in zip(sigm,zip(preds, out_label_ids)):
                writer.write("%s\t %s\t %s\n" % (pr, gt, prob))
        result = compute_metrics(eval_task.lower(), preds, out_label_ids)
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False, evaluate_on_test=False):
    ...
    cached_features_file = os.path.join(args.data_dir, 'cached_{}_{}_{}_{}'.format(
        args.evaluation_set if evaluate else 'train',
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        elif args.evaluation_set == 'test_synthetic':
            examples = processor.get_test_synth_examples(args.synth_data_dir) 
        elif args.evaluation_set == 'unrelated':
            examples = processor.get_unrelated_examples(args.synth_data_dir)
        elif args.evaluation_set == 'some_meaning_difference':
            examples = processor.get_some_meaning_difference_examples(args.synth_data_dir) 
        ...
        features = convert_examples_to_features(examples,
                                                ...
                                                test=True if evaluate_on_test else False,
        )
    ...
    if output_mode == "classification":
        if not evaluate_on_test:
            all_labels_a = torch.tensor([f.label_a for f in features], dtype=torch.long)
            all_labels_b = torch.tensor([f.label_b for f in features], dtype=torch.long)
        else:
            all_labels_a = torch.tensor([f.label for f in features], dtype=torch.long)
    elif output_mode == "regression":
        logger.info('Margin loss not implemented for regression tasks... Abort')
        exit(0)

    if not evaluate_on_test:
        dataset = TensorDataset(all_input_ids_a, all_attention_mask_a, all_token_type_ids_a, all_labels_a,\
                           all_input_ids_b, all_attention_mask_b, all_token_type_ids_b, all_labels_b)
    else:
        dataset = TensorDataset(all_input_ids_a, all_attention_mask_a, all_token_type_ids_a, all_labels_a)
    return dataset
...
def main():
    ...
    parser.add_argument("--evaluate_on_training", action='store_true',
                        help="Rul evaluation on training set at each logging step.")
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        if args.best_checkpoint:
            ...
            for checkpoint in checkpoints:
                ...
                if args.evaluation_set == 'dev':
                    result = evaluate(args, model, tokenizer, prefix, prefix=prefix)
                else:
                    result = evaluate(args, model, tokenizer, prefix, prefix=prefix, evaluate_on_test=True)

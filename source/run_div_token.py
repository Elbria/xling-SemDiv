...
from utils_token_div import convert_examples_to_features, get_labels, read_examples_from_file
...
def subword2token_labels(args, output_test_predictions_file, predictions, tokenizer, mode='Test'):

    label_map = {'CHA': 'D', 'ADD': 'D', 'O': 'O', 'D': 'D'}
    binarize_map = {'CHA': 1, 'ADD': 1, 'D': 1, 'O': 0}

    total_predictions_src, total_gold_src = [], []
    total_predictions_tgt, total_gold_tgt = [], []

    if mode=='Test':
        file_ = "test_synth.tsv"
    else:
        file_ = "dev.tsv"

    with open(output_test_predictions_file, "w") as writer:
        with open(os.path.join(args.data_dir, file_), "r") as f:
            f = f.readlines()
            example_id = -1
...
def main():
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for id_,checkpoint in enumerate(checkpoints):
            global_step = checkpoint.split("-")[-1] if len(checkpoints) > 1 else ""
            model = model_class.from_pretrained(checkpoint)
            model.to(args.device)

            _, predictions = evaluate(args, model, tokenizer, labels, pad_token_label_id, mode="dev", prefix=global_step)
            output_dev_predictions_file = os.path.join(args.output_dir, "dev_predictions.txt")
            result = subword2token_labels(args, output_dev_predictions_file, predictions, tokenizer, mode='dev')
    ...
    if args.do_predict and args.local_rank in [-1, 0]:
        ...
        model.to(args.device)
        result, predictions = evaluate(args, model, tokenizer, labels, pad_token_label_id, mode="test_synth")
        ...
        results = subword2token_labels(args, output_test_predictions_file, predictions, tokenizer, mode='Test')
